...
/**
 * Facet query that computes the sum of the specified field.
 *
...
public class SumFacetQuery extends AbstractStatsFacetQuery {

	/**
	 * Creates a `sum` facet query.
	 *
	 * @param id Facet identifier; produces a corresponding entry in the response.
	 * @param field Index field whose values are summed.
	 * @param precision Decimal precision to apply when formatting numeric results. May be null to use default precision.
	 */
	@JsonCreator public SumFacetQuery(@JsonProperty("id") String id, @JsonProperty("field") String field, @JsonProperty("precision") Integer precision) {
		super(id, FacetType.SUM, field, precision);
	}
}
