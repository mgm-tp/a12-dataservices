...
/**
 * Facet query that computes the maximum of the specified field.
 *
...
public class MaxFacetQuery extends AbstractStatsFacetQuery {

	/**
	 * Creates a `max` facet query.
	 *
	 * @param id Facet identifier; produces a corresponding entry in the response.
	 * @param field Index field whose maximum value is computed.
	 * @param precision Decimal precision to apply when formatting numeric results. May be null to use default precision.
	 */
	@JsonCreator public MaxFacetQuery(@JsonProperty("id") String id, @JsonProperty("field") String field, @JsonProperty("precision") Integer precision) {
		super(id, FacetType.MAX, field, precision);
	}
}
