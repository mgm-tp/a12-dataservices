...
public class TestResourceRelationshipModelLoader extends AbstractTestResourcesModelLoader<RelationshipModel> implements RelationshipModelLoader {

	private final RelationshipModelSerializer relationshipModelSerializer;

	@Override protected RelationshipModel deserializeModel(@NonNull Header h, @NonNull Reader r) {
		return relationshipModelSerializer.deserialize(r, h);
	}

	@SneakyThrows
	@Override public Set<RelationshipModel> loadAllRelationshipModels() {
		return Arrays.stream(resourcePatternResolver.getResources("classpath*:/models/**/*.json"))
			.map(this::toHeaderAndContent)
			.filter(p -> RELATIONSHIP_MODEL_TYPE.equals(p.getLeft().getModelType()))
