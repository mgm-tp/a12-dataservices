...
@QueryOperatorValidator(NestingOperator.class)
@Component public class NestingOperatorValidator implements IQueryOperatorValidator {
	...
	@Override public @NonNull Collection<ValidationItem> validate(ILogicOperator operator, String parentDocumentModel, String[] path, QueryContext context, boolean validationEnabled) {
		if (validationEnabled && operator instanceof NestingOperator nestingOperator) {
			if (nestingOperator.getOperand() == null) {
				return List.of(ValidationItem.invalid(path, "Please provide operand for %s operator.".formatted(context.getOperatorName(operator))));
			} else {
