 ...
 */
package com.mgmtp.a12.dataservices.configuration.condition.internal;
...
public abstract class AbstractDataServicesCondition extends SpringBootCondition {
	 ...
	 * Falls back to a default instance when no `mgmtp.a12.dataservices.*` property is present.
	 *
	 * @param environment spring {@link Environment}
	 * @return actual {@link DataServicesCoreProperties}, never `null`
	 */
	public static @NonNull DataServicesCoreProperties findBoundProperties(Environment environment) {
		return Optional.of(environment)
			...
			.map(BindResult::get)
			.orElse(new DataServicesCoreProperties());
	}
}
