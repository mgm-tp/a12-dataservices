...
@SuperBuilder
public class LinkColumnsGenerator extends AbstractLinkColumnsGenerator {

	@Override public CharSequence getTypeValue() {
		return SqlGeneratorHelpers.addParam(QueryGeneratorConstants.NodeType.CHILD_TYPE, getQueryGeneratorContext());
	}

	@Override public CharSequence getContentValue(QueryGeneratorContext generatorContext) {
		// If no fields selection is present in document projection, no content should be loaded.
		return DocumentProjectionImplementation.PROJECTION_NAME.equals(generatorContext.getProjectionName()) && CollectionUtils.isEmpty(getQuery().getFields()) ?
			null :
			QueryGeneratorConstants.ColumnNames.ORIGINAL_VALUE_COLUMN_NAME;
	}

	@Override public CharSequence getCountValue() {
		if (getQueryGeneratorContext().isExclude()) {
			return QueryGeneratorConstants.ColumnNames.LINK_COUNT_COLUMN_ALIS;
		} else {
			return super.getCountValue();
		}
	}
}
