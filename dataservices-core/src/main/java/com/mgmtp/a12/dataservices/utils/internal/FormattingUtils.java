...
public class FormattingUtils {
	public static String toYaml(Collection<ValidationItem> errors) {
		return errors.stream()
			.map(e -> """
				- path: %s
				  message: |
				    %s
				""".formatted(constructJsonPathDefinition(e.path()), e.message().replace("\n", "\n    ")))
			.collect(Collectors.joining("\n"));
	}

	@NotNull private static String constructJsonPathDefinition(@NonNull String[] argument) {
		return String.join(".", argument).replace(".[", "[");
	}
}
