/*
 * SPDX-License-Identifier: EUPL-1.2 OR LicenseRef-commercial
 *
 * Copyright (c) 2012-2026 mgm technology partners GmbH
 *
 * Dual License
 * ------------
 * This source file is part of the mgm A12 Platform and available under
 * a choice of two different licenses:
 *
 * 1. Open-Source License – EUPL v1.2
 *    You may redistribute and/or modify this file under the terms of the
 *    European Union Public License, version 1.2 - see https://eupl.eu/.
 *
 * 2. Commercial License
 *    Alternatively, you may obtain a commercial license from
 *    mgm technology partners GmbH, that permits use of this software
 *    under different terms (including support and maintenance services).
 *
 *    Please contact a12-license@mgm-tp.com for more information.
 *
 * You must select and comply with exactly one of the above license options.
 *
 * Warranty Disclaimer (applies to either option)
 * ----------------------------------------------
 * THIS SOFTWARE IS PROVIDED “AS IS” AND WITHOUT WARRANTY OF ANY KIND,
 * WHETHER EXPRESS OR IMPLIED, INCLUDING BUT NOT LIMITED TO THE WARRANTIES
 * OF MERCHANTABILITY, FITNESS FOR A PARTICULAR PURPOSE AND
 * NON-INFRINGEMENT, EXCEPT WHERE SUCH DISCLAIMERS ARE HELD TO BE
 * LEGALLY INVALID. SEE THE RESPECTIVE LICENSE TEXT FOR DETAILS.
 */
package com.mgmtp.a12.dataservices.model.persistence.internal;

import org.apache.commons.lang3.time.StopWatch;
import org.springframework.context.ApplicationEventPublisher;

import com.mgmtp.a12.dataservices.authorization.ModelPermissionEvaluator;
import com.mgmtp.a12.dataservices.internal.DocumentationDiagram;
import com.mgmtp.a12.dataservices.model.events.ModelAfterLoadEvent;
import com.mgmtp.a12.dataservices.model.persistence.IModelLoader;
import com.mgmtp.a12.dataservices.model.persistence.IModelReadRepository;
import com.mgmtp.a12.model.Model;

import lombok.NonNull;
import lombok.RequiredArgsConstructor;
import lombok.extern.slf4j.Slf4j;

@DocumentationDiagram
@Slf4j @RequiredArgsConstructor
public abstract class AbstractModelLoader<T extends Model> implements IModelLoader<T> {

	protected final ModelPermissionEvaluator<T> modelPermissionEvaluator;
	protected final IModelReadRepository<T> modelReadRepository;
	private final ApplicationEventPublisher eventPublisher;

	protected T loadModelInsecure(@NonNull String modelId) {
		return modelReadRepository.readModel(modelId);
	}

	/**
	 *
	 * @param modelId The model id.
	 * @return The typed loaded model.
	 * @event {@link ModelAfterLoadEvent}
	 */
	@Override public T loadModel(@NonNull String modelId) {
		StopWatch stopWatch = StopWatch.createStarted();
		T model = loadModelInsecure(modelId);
		modelPermissionEvaluator.checkModelReadPermission(model);
		log.debug("Model [{}] has been successfully loaded in [{}] ms", model.getHeader().getId(), stopWatch.getTime());
		ModelAfterLoadEvent<T> modelAfterLoadEvent = new ModelAfterLoadEvent<>(model);
		eventPublisher.publishEvent(modelAfterLoadEvent);
		return model;
	}
}
