...
@QueryOperatorValidator(SimpleSearchOperator.class)
@Component public class SimpleSearchOperatorValidator implements IQueryOperatorValidator {
	...
	@Override public @NonNull Collection<ValidationItem> validate(ILogicOperator operator, String parentDocumentModel, String[] path, QueryContext context, boolean validationEnabled) {
		if (validationEnabled && operator instanceof SimpleSearchOperator simpleSearchOperator) {
			if (simpleSearchOperator.getValue() == null && isEmpty(simpleSearchOperator.getValues())) {
