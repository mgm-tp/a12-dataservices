...
public class RpcExceptionMapper {
		public static <T> T mapRpcValidationException(Callable<T> r, String operationId) {
			try {
				return r.call();
			} catch (RelationshipValidationException e) {
				throw RpcExceptionSupport.createException(e.getCode(), e.getKeyPrefix(), e.getTitle(), e.getMessage(), operationId);
			} catch (RuntimeException e) {
				throw e;
			} catch (Exception e) {
				throw new UnexpectedException(e).withAnonymityMessage("Rpc validation failed.");
			}
		}
}
