...
/**
 * @deprecated since 38.1.0 , This class will be moved to internal package in future release
 *
		...
		if (dataServicesCoreProperties.getModels().getRelationship().getSafeDelete().isEnabled()) {
			...
			if (isRelationshipModel(model) && relationshipLinkService.countByRelationshipModel(model.getHeader().getId()) > 0) {
				throw new InvalidInputException(ExceptionKeys.RELATIONSHIP_MODEL_DELETE_LINK_EXISTS_ERROR_KEY,
					String.format("Relationship model %s cannot be deleted because links for that model exist in the persistent store",
		...
		try {
			...
			if (!results.noErrorOccurred()) {
				throw new InvalidInputException(ExceptionKeys.MODEL_VALIDATION_ERROR_KEY,
					String.format("Relationship model [%s] is not valid:%n%s", header.getId(), results.getMessages())).withAnonymityMessage("Relationship model is not valid.");
			}
		} catch (DocumentSerializationException | JsonProcessingException e) {
			throw new InvalidInputException(ExceptionKeys.MODEL_VALIDATION_ERROR_KEY, String.format("Relationship model [%s] is not valid", header.getId()), e);
		}
	}
	...
	private void validateRelationshipModelVersion(Header header) {
		if (!RelationshipModel.VERSION.equals(header.getModelVersion())) {
			throw new InvalidInputException(ExceptionKeys.MODEL_VERSION_MISMATCH_ERROR_KEY,
				String.format("Currently supported version for relationship models is %s", RelationshipModel.VERSION));
		}
	}
