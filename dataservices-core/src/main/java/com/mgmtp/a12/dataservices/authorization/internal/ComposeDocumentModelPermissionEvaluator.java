...
@Component public class ComposeDocumentModelPermissionEvaluator extends AbstractModelPermissionEvaluator<ComposeDocumentModel> {
	...
	public ComposeDocumentModelPermissionEvaluator(AuthorizationService authorizationService, CachedPermissionEvaluator cachedPermissionEvaluator,
		ModelHeaderJpaRepository modelHeaderRepository, ModelPermissionEvaluator<RelationshipModel> relationshipModelPermissionEvaluator,
		ModelPermissionEvaluator<IDocumentModel> modelPermissionEvaluator) {
	...
	@Override public void checkModelReadPermission(ComposeDocumentModel model) {
		super.checkModelReadPermission(model);
		checkCrdPermissions(model.getHeader());
		checkNestedModelsPermissions(model);
	}

	private void checkCrdPermissions(Header header) {
		modelPermissionEvaluator.checkModelReadPermission(ComposeDocumentModelUtils.getCrdModelName(header));
	}

	private void checkNestedModelsPermissions(ComposeDocumentModel model) {
		ComposeDocumentModelUtils.getCdmSkeletonGroups(model)
			.map(CddSkeletonGroup::getRelationshipModelName)
			.filter(Objects::nonNull)
			.forEach(relationshipModelPermissionEvaluator::checkModelReadPermission);
	}
}
