...
@Component public class DefaultRelationshipModelSerializer implements RelationshipModelSerializer {
	...
	@Override public RelationshipModel deserialize(@NonNull Reader modelContent, @NonNull Header header) {
		try {
			...
			RelationshipValidationSupport.validateRelationshipModelVersion(relationshipModel);
	...
	@Override public String serialize(@NonNull RelationshipModel relationshipModel) {
		try {
			...
			throw new ModelSerializationException(String.format("Unable to serialize RelationshipModel %s", relationshipModel.getHeader().getId()), e);
		}
	}
	...
	@Component public static class HeaderSerializer extends JsonSerializer<Header> {
		private final HeaderJsonSerializer headerJsonSerializer = new DefaultHeaderJsonSerializer();

		@Override public void serialize(Header value, JsonGenerator gen, SerializerProvider serializers) throws IOException {
