...
public class CachedPermissionEvaluatorTest extends AbstractCorePermissionTest {

	@InjectMocks private CachedPermissionEvaluator cachedPermissionEvaluator;



	@Test (dataProvider = "permissionResultCheck")
	void testHasReadPermission_success(boolean result) {
		Header header = makeTestModelHeader();
		Mockito.when(authorizationService.checkPermissions(Mockito.any(), Mockito.any()))
			.thenReturn(createPermissionCheckResult(result));

		Assert.assertEquals(cachedPermissionEvaluator.hasModelReadPermission(header), result);
