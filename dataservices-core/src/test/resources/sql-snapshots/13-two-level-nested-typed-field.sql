...
--   ]
-- }
-- field type: IStringType  →  sorts via d2.original_value (terminal document alias, no df2 reference)
-- params: p0..p2=hop1(model,sourceRole,targetRole), p3..p5=hop2, p6=fieldName; p7=type, p8=internal_id
WITH "Contract_0001" AS (SELECT cte_root.*,
  ROW_NUMBER() OVER (ORDER BY
    d2.original_value #>> '{"AddressRoot","City"}' COLLATE "C"  NULLS LAST
  ) AS _sort_rank
  ...
  LEFT JOIN (SELECT source_role.role_docref AS source_docref, target_role.role_docref AS target_docref
    FROM relationship_role AS source_role, relationship_link AS link, relationship_role AS target_role
    WHERE link.relationship_model = :p3
      AND source_role.relationship_id = link.id
      AND link.id = target_role.relationship_id
      AND source_role.role_name = :p4
      AND target_role.role_name = :p5
  ) AS rr2 ON rr2.source_docref = d1.doc_ref
  LEFT JOIN document_search AS d2 ON d2.doc_ref = rr2.target_docref
  LEFT JOIN document_fields AS df2 ON df2.doc_ref = d2.doc_ref AND df2.field_name = :p6
  ORDER BY _sort_rank OFFSET 0 LIMIT 0)SELECT doc_ref :: TEXT AS doc_ref,model_name :: TEXT AS model_name,original_value :: JSONB AS content,'' :: TEXT AS relationship_model,'' :: TEXT AS source_role,'' :: TEXT AS source_docref,'' :: TEXT AS target_role,'' :: TEXT AS target_docref,NULL :: TEXT AS link_document,'' :: TEXT AS link_id,:p7 :: TEXT AS type,'' :: TEXT AS back_reference,:p8 :: TEXT AS internal_id,FALSE :: BOOLEAN AS fields_projection,0 :: NUMERIC AS depth, ROW_NUMBER () OVER ()  AS row_num,total_count :: NUMERIC AS total_count
