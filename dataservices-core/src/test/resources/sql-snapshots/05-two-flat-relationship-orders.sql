...
--     {
--       "relationshipField": {
--         "relationshipModel": "ContractInsurer",
--         "targetRole": "Insurer",
--         "field": "companyName"
--       },
--       "direction": "ASC",
...
--   ]
-- }
-- Both sort orders are independent; each anchors on cte_root.doc_ref (not on d1.doc_ref).
-- params: p0..p3=sort1(model,sourceRole,targetRole,field), p4..p7=sort2; p8=type, p9=internal_id
WITH "Contract_0001" AS (SELECT cte_root.*,
  ROW_NUMBER() OVER (ORDER BY
    d1.original_value #>> '{"name"}' NULLS LAST ,
    d2.original_value #>> '{"companyName"}' NULLS LAST
  ) AS _sort_rank
