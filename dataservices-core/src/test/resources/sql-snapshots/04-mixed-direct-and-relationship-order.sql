...
--   ]
-- }
-- direct field order generates no JOIN; relationship order generates one derived table JOIN (hop 1)
-- params: p0=model, p1=sourceRole, p2=targetRole, p3=fieldName; p4=type, p5=internal_id
WITH "Contract_0001" AS (SELECT cte_root.*,
  ROW_NUMBER() OVER (ORDER BY
    cte_root.original_value #> '{"contractNumber"}',
    d1.original_value #>> '{"name"}' DESC  NULLS FIRST
  ) AS _sort_rank
