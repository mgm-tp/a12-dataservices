...
public class SeedDataServiceTestHelper {

	protected static final DefaultResourceLoader DEFAULT_RESOURCE_LOADER = new DefaultResourceLoader();

	public static void createSeedDataTar(OutputStream outputStream, String attachmentId, String documentId, String linkId, boolean includedModels,
		...
		try (
			...
			if (includeMeta) {
				writeFile(tars, SeedDataService.FULL_META_PATH,
					DEFAULT_RESOURCE_LOADER.getResource("classpath:import/data/meta/seed_metadata.json").getContentAsByteArray());
			}
			if (includedModels) {
