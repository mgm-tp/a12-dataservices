...
import com.mgmtp.a12.dataservices.model.SeedRoleSpec;
...
@Component public class LinkExporter extends AbstractTarExporter<Void> {
	...
	private void exportForRelationshipModel(TarArchiveOutputStream taos, RelationshipModel relationshipModel) {
		relationshipLinkJpaRepository.findByRelationshipModel(relationshipModel.getHeader().getId(), null)
			...
			.forEach(link -> {
				try {
					writeFileToTar(taos, objectMapper.writeValueAsBytes(createSeedLinkDescriptor(link)), constructTarPah(relationshipModel, link));
	...
	private static SeedLinkDescriptor createSeedLinkDescriptor(RelationshipLinkEntity entity) {

		SeedLinkDescriptor linkDescriptor = new SeedLinkDescriptor();
	...
	@NonNull private static SeedRoleSpec roleEntityRoSeedRoleSpec(RelationshipLinkEntity entity, String key) {
		RelationshipRoleEntity roleEntity = entity.getRoles().get(key);
		return new SeedRoleSpec(roleEntity.getName(), roleEntity.getDocRef(), roleEntity.getOrder());
	}
}
