...
public class RpcAddLinkOrderIT extends RpcOrderedLinkIT {
	...
	@Transactional
	@Test public void insertWithOrder1() throws IOException {
		...
		String request = template.formatted(RoleConstants.PARTNER_ROLE, partner1DocRef, RoleConstants.CONTRACT_ROLE, contract1DocRef, "");
		...
		request = template.formatted(RoleConstants.PARTNER_ROLE, partner1DocRef, RoleConstants.CONTRACT_ROLE, contract2DocRef, firstLink);
		sendRpcRequest(request);
		...
		List<DocumentTreeResult> links = result.getLinks().stream()
			.filter(link -> link.getType() == DocumentTreeNodeType.LINK)
			.toList();
		Assert.assertEquals(links.getFirst().getLinkId(), firstLink);
		String secondLink = links.get(1).getLinkId();

		request = template.formatted(RoleConstants.PARTNER_ROLE, partner1DocRef, RoleConstants.CONTRACT_ROLE, contract3DocRef, secondLink);
		sendRpcRequest(request);
		...
		request = template.formatted(RoleConstants.PARTNER_ROLE, partner1DocRef, RoleConstants.CONTRACT_ROLE, contract4DocRef, firstLink);
		sendRpcRequest(request);
		...
		request = template.formatted(RoleConstants.PARTNER_ROLE, partner1DocRef, RoleConstants.CONTRACT_ROLE, contract5DocRef, "");
		sendRpcRequest(request);
		...
		links = result.getLinks().stream()
			.filter(link -> link.getType() == DocumentTreeNodeType.LINK)
			.toList();
		String fifthLink = links.getFirst().getLinkId();
	...
	public void insertWithOrder2() throws IOException {
		String template = loadResourceFromClasspathAsString(PathConstants.RPC_PATH + "add/add_link_head.json");
		// no predecessor init request
		String request = template.formatted(RoleConstants.CONTRACT_ROLE, contract1DocRef, RoleConstants.PARTNER_ROLE, partner1DocRef, "");
		sendRpcRequest(request);
		...
		request = template.formatted(RoleConstants.CONTRACT_ROLE, contract1DocRef, RoleConstants.PARTNER_ROLE, partner2DocRef, firstLink);
		sendRpcRequest(request);
		...
		request = template.formatted(RoleConstants.CONTRACT_ROLE, contract1DocRef, RoleConstants.PARTNER_ROLE, partner3DocRef, secondLink);
		sendRpcRequest(request);
		...
		request = template.formatted(RoleConstants.CONTRACT_ROLE, contract1DocRef, RoleConstants.PARTNER_ROLE, partner4DocRef, firstLink);
		sendRpcRequest(request);
		...
		request = template.formatted(RoleConstants.CONTRACT_ROLE, contract1DocRef, RoleConstants.PARTNER_ROLE, partner5DocRef, "");
		sendRpcRequest(request);
	...
	public void testWithSourceRoleOrderOnly() throws IOException {
		String template = loadResourceFromClasspathAsString(PathConstants.RPC_PATH + "add/add_link_head.json");
		// no predecessor init request
		sendRpcRequest(template.formatted(RoleConstants.PARTNER_ROLE, partner1DocRef, RoleConstants.CONTRACT_ROLE, contract3DocRef, ""));
		sendRpcRequest(template.formatted(RoleConstants.PARTNER_ROLE, partner1DocRef, RoleConstants.CONTRACT_ROLE, contract2DocRef, ""));
		sendRpcRequest(template.formatted(RoleConstants.PARTNER_ROLE, partner1DocRef, RoleConstants.CONTRACT_ROLE, contract1DocRef, ""));
		...
		Assert.assertEquals(sourceRoleOrderResult.getLinks().getFirst().getTargetDocRef(), contract1DocRef);
		...
		partnerRole.put("ordered", false);
		modelsFunctions.updateModelContent(node.toString());
		modelCacheManager.invalidateModelReadCaches(RelationshipModelConstants.CONTRACT_COINSURED_BUSINESS_PARTNER_MODEL);

		PagedResultSet<DocumentTreeResult> orderedByLinkIdResult = queryOperation.rpc(queryLink);
		List<DocumentTreeResult> childItemsByLinkId = orderedByLinkIdResult.getLinks().stream()
			.filter(d -> d.getType() == DocumentTreeNodeType.CHILD)
			.toList();
		List<DocumentTreeResult> sortedByLinkId = new ArrayList<>(childItemsByLinkId);
		sortedByLinkId.sort((d1, d2) -> d1.getLinkId().compareToIgnoreCase(d2.getLinkId()));

		Assert.assertEquals(orderedByLinkIdResult.getFullSize(), 3);
		Assert.assertEquals(childItemsByLinkId.get(0).getTargetDocRef(), sortedByLinkId.get(0).getTargetDocRef());
		Assert.assertEquals(childItemsByLinkId.get(1).getTargetDocRef(), sortedByLinkId.get(1).getTargetDocRef());
		Assert.assertEquals(childItemsByLinkId.get(2).getTargetDocRef(), sortedByLinkId.get(2).getTargetDocRef());

		// modify source role order to true, target role order to false, it will keep order by source role order.
		partnerRole.put("ordered", true);
		contractRole.put("ordered", false);
		modelsFunctions.updateModelContent(node.toString());
		modelCacheManager.invalidateModelReadCaches(RelationshipModelConstants.CONTRACT_COINSURED_BUSINESS_PARTNER_MODEL);
		...
		Assert.assertEquals(orderedResult.getLinks().getFirst().getTargetDocRef(), contract1DocRef);
	...
	public void vwrongPredecessor() throws IOException {
		String template = loadResourceFromClasspathAsString(PathConstants.RPC_PATH + "add/add_link_head.json");
		// no predecessor init request
		String request = template.formatted(RoleConstants.PARTNER_ROLE, partner2DocRef, RoleConstants.CONTRACT_ROLE, contract1DocRef, "9999999999");
		sendRpcRequest(request);

		request = template.formatted(RoleConstants.PARTNER_ROLE, partner2DocRef, RoleConstants.CONTRACT_ROLE, contract2DocRef, "9999999999");
