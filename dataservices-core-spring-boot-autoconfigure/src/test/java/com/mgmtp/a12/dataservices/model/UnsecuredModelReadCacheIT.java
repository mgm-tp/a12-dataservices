...
public class UnsecuredModelReadCacheIT extends AbstractSpringContextIT {

	@Autowired private ModelCacheManager modelCacheManager;
	...
	public void secondCallHitsCache() {
		...
		modelResolverSpy.getDocumentModelSearchService(BUSINESS_PARTNER_DOCUMENT_MODEL);
		Mockito.verify(modelResolverSpy, Mockito.times(1)).getDocumentModelSearchService(anyString());
	}
	...
	public void differentCallsInvokeMethod() {
		...
		Mockito.verify(modelResolverSpy, Mockito.times(2)).getDocumentModelSearchService(anyString());
	}
