...
public class JsonRpcInitializerIT extends AbstractSpringContextIT {
	...
	public void invalidPathWithError() throws Exception {
		memoryAppender.reset();
		getInitializer(INVALID_PATH).execute();
	}

	@Test
	public void singleFirstFile() throws Exception {
		memoryAppender.reset();
	...
	public void singleThirdFileWithError() throws Exception {
		memoryAppender.reset();
		getInitializer(FILE_3_PATH).execute();
	}

	@Test(expectedExceptions = RuntimeException.class)
	public void passFolderWithError() throws Exception {
		memoryAppender.reset();
		getInitializer(FOLDER_PATH_ERROR).execute();
	}

	@Test
	public void passFolderShouldExecuteInOrderAndCorrectly() throws Exception {
		memoryAppender.reset();
	...
	@Test
	public void addDocumentAndReferenceItToDelete() throws Exception {
		memoryAppender.reset();
	...
	private static class MemoryAppender extends ListAppender<ILoggingEvent> {
		...
		private void assertMessage(String[] args, String jsonRpcSuccessForPath) {
			...
			throw error.map(AssertionError::new)
				.orElseThrow(() -> new AssertionError(String.format("NO SUCCESS MESSAGE FOUND:\n%s", this.list.stream()
		...
		private Optional<String> assertArgs(Object[] argumentArray, Pattern[] args) {
			for (int i = 0; i < args.length; i++) {
				if (!args[i].matcher(argumentArray[i].toString()).matches()) {
					return Optional.of(String.format("ARG:\n%s\nPATTERN:\n%s", argumentArray[i], args[i]));
				}
			}
