...
public class OnEnabledRpcCondition extends AbstractDataServicesCondition {

	private static final String RPC_ENABLED_CONFIGURATION = "mgmtp.a12.dataservices.jsonRpc.enabled";
	...
	@Override protected boolean evaluateCondition(DataServicesCoreProperties dataServicesCoreProperties) {
		return dataServicesCoreProperties.getJsonRpc().isEnabled();
	}
	...
	private ConfigurationMessage enabledMessage() {
		return makeValidMessage("RPC is enabled", RPC_ENABLED_CONFIGURATION);
	}

	private ConfigurationMessage disabledMessage() {
		return makeValidMessage("RPC is disabled", RPC_ENABLED_CONFIGURATION);
	}
}
