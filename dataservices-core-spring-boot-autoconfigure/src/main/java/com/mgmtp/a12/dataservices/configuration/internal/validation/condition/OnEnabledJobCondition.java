...
public class OnEnabledJobCondition extends AbstractDataServicesCondition {
	...
	private static final String JOBS_ENABLED = "mgmtp.a12.dataservices.jobs.enabled";
	...
	@Override protected boolean evaluateCondition(DataServicesCoreProperties dataServicesCoreProperties) {
		return dataServicesCoreProperties.getJobs().isEnabled();
	}
	...
	private ConfigurationMessage enabledMessage() {
		return makeValidMessage("QUARTZ Job is enabled", JOBS_ENABLED);
	}

	private ConfigurationMessage disabledMessage() {
		return makeValidMessage("QUARTZ Job is disabled", JOBS_ENABLED);
	}
}
