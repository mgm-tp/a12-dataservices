...
public class ReindexingCondition extends AbstractDataServicesCondition {
	...
	@Override protected boolean evaluateCondition(DataServicesCoreProperties dataServicesCoreProperties) {
		return dataServicesCoreProperties.getQuery().getReindexing().getMode().equals(DataServicesCoreProperties.Query.Reindexing.Mode.REBUILD_INDEX);
	}
	...
	private ConfigurationMessage enabledMessage() {
		return makeValidMessage("Query re-indexing mode is REBUILD_INDEX", REINDEXING_CONFIG);
	}

	private ConfigurationMessage disabledMessage() {
		return makeValidMessage("Query re-indexing mode is not REBUILD_INDEX", REINDEXING_CONFIG);
	}
}
