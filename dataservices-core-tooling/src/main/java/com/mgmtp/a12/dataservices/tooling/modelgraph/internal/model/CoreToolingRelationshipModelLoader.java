...
@Component public class CoreToolingRelationshipModelLoader implements RelationshipModelLoader {

	private final CoreToolingModelService coreToolingModelService;
	...
	@Override public Set<RelationshipModel> loadAllRelationshipModels() {
		return coreToolingModelService.getModels().entrySet().stream()
			.filter(e -> RELATIONSHIP_MODEL_TYPE.equals(e.getKey().getModelType()))
			.map(this::constructRelationshipModel)
			.collect(Collectors.toSet());
	}

	@Override public RelationshipModel loadModel(@NotNull String relationshipModelName) {
		throw new UnsupportedOperationException();
	}

	@SneakyThrows
	private RelationshipModel constructRelationshipModel(Map.Entry<Header, String> entry) {
		Header header = entry.getKey();
		String rawContent = entry.getValue();
		return relationshipModelSerializer.deserialize(new StringReader(rawContent), header);
	}
}
