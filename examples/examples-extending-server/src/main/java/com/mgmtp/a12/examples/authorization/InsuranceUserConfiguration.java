...
public class InsuranceUserConfiguration {
	 ...
	 *
	 * @return a {@link LocalUserLoader} for {@link InsuranceUser}.
	 */
	@Bean public LocalUserLoader<InsuranceUser> localInsuranceUserLoader() {
		return new LocalUserLoader<>() {
			@Override public InsuranceUser loadUser(Resource resource) {
				try {
					return usersDeserializer.readValue(resource.getInputStream(), InsuranceUser.class);
				} catch (Exception e) {
					LOGGER.error("Unable to load user from resource [{}]", resource.getFilename(), e);
					return null;
				}
			}
		};
