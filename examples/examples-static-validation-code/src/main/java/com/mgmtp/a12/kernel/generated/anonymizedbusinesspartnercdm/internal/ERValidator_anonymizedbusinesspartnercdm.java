...
public class ERValidator_anonymizedbusinesspartnercdm
	...
	private static final IMetaDataValidierung metaData = Meta_anonymizedbusinesspartnercdm.getInstance().getMetaDataValidierung();
	private static final ERValidator_anonymizedbusinesspartnercdm instance = new ERValidator_anonymizedbusinesspartnercdm();

	private ERValidator_anonymizedbusinesspartnercdm () {
	}

	public static ERValidator_anonymizedbusinesspartnercdm getInstance() {
		return instance;
	}
	...
	private void pruefeParams(
		...
		if (modus != null && !Meta_anonymizedbusinesspartnercdm.getInstance().isSpracheUnterstuetzt(modus.getLanguage())) {
	...
	public void validatePartlyWith3ValueLogic(
		...
		ValidationCommand c = new ValidationCommand(R_anonymizedbusinesspartnercdm.getInstance(), modus, metaData, daten);
	...
	public ICalculationResult calculateAll(IData daten, ValidationMode modus, IResult ergebnis) {
		pruefeParams(daten, ergebnis, modus);
		CalculationCommand c = new CalculationCommand(R_anonymizedbusinesspartnercdm.getInstance(), modus, metaData, daten, C_anonymizedbusinesspartnercdm.getInstance());
