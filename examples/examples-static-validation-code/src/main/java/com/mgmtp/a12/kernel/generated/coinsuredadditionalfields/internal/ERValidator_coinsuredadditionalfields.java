...
public class ERValidator_coinsuredadditionalfields
	...
	private static final IMetaDataValidierung metaData = Meta_coinsuredadditionalfields.getInstance().getMetaDataValidierung();
	private static final ERValidator_coinsuredadditionalfields instance = new ERValidator_coinsuredadditionalfields();

	private ERValidator_coinsuredadditionalfields () {
	}

	public static ERValidator_coinsuredadditionalfields getInstance() {
		return instance;
	}
	...
	private void pruefeParams(
		...
		if (modus != null && !Meta_coinsuredadditionalfields.getInstance().isSpracheUnterstuetzt(modus.getLanguage())) {
	...
	public void validatePartlyWith3ValueLogic(
		...
		ValidationCommand c = new ValidationCommand(R_coinsuredadditionalfields.getInstance(), modus, metaData, daten);
	...
	public ICalculationResult calculateAll(IData daten, ValidationMode modus, IResult ergebnis) {
		pruefeParams(daten, ergebnis, modus);
		CalculationCommand c = new CalculationCommand(R_coinsuredadditionalfields.getInstance(), modus, metaData, daten, C_coinsuredadditionalfields.getInstance());
