 ...
 *  
 */
public class TVCheckFehler_recursivebusinesspartnercdm implements ITeilValidierungChecker {

	private R_recursivebusinesspartnercdm checkObject = R_recursivebusinesspartnercdm.getInstance();

	public TVCheckFehler_recursivebusinesspartnercdm() {
	}

	public void fuehreAus(MainValidatorController controller, int interneFeldNr, long[] indices) {
		if(interneFeldNr<=13) { fuehreAus_0(controller, interneFeldNr, indices); }
	...
	private void fuehreAus_0(MainValidatorController controller, int interneFeldNr, long[] indices) {
		...
		default:
			if(interneFeldNr<=13) {
				...
				throw new IllegalArgumentException("Die interne Feldnummer "+interneFeldNr+" liegt nicht im Interval [1,13].");
			}
		}
	}
