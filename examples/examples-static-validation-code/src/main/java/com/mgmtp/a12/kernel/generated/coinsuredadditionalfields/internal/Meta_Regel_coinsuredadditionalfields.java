 ...
 *
 */
public class  Meta_Regel_coinsuredadditionalfields {
	...
	refIdentifier[0] = new Identifier_t[]{
			new Identifier_t( 1, new long[]{1, 1}, IIdentifier.USB_NOT_SPECIFIED, ReferenzTyp.KEINE_BERECHNUNG, null, null, "/CoInsuredRoot/ID")
	};
...

	{
	    // filter (decl & refs) omitted for referenced entities
	  	final RtIdentifierTemplate metaRule0RefsrtIdCon1 = RtIdentifierTemplate.builder().unqNm("ID").isField(true).usb(-2).idx(1).idx(1).build();
	...
	regeln.add(new Regel_t("AddID", "/CoInsuredRoot/AddID", "AddID", mehrsprachigeFehlertexte.get(0), "Fehler", refIdentifier[0], refAuslassungsIdentifier[0], refEntity[0], refAuslassungsEntity[0], null,"/CoInsuredRoot/ID", true, false, new MapBuilder<String, Object>().build(), Set.of()));
	}


  	public Meta_Regel_coinsuredadditionalfields() {
        initRegelRefs_0();
