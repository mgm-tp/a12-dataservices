...
public class ERValidator_relationshipmetamodel
	...
	private static final IMetaDataValidierung metaData = Meta_relationshipmetamodel.getInstance().getMetaDataValidierung();
	private static final ERValidator_relationshipmetamodel instance = new ERValidator_relationshipmetamodel();

	private ERValidator_relationshipmetamodel () {
	}

	public static ERValidator_relationshipmetamodel getInstance() {
		return instance;
	}
	...
	private void pruefeParams(
		...
		if (modus != null && !Meta_relationshipmetamodel.getInstance().isSpracheUnterstuetzt(modus.getLanguage())) {
	...
	public void validatePartlyWith3ValueLogic(
		...
		ValidationCommand c = new ValidationCommand(R_relationshipmetamodel.getInstance(), modus, metaData, daten);
	...
	public ICalculationResult calculateAll(IData daten, ValidationMode modus, IResult ergebnis) {
		pruefeParams(daten, ergebnis, modus);
		CalculationCommand c = new CalculationCommand(R_relationshipmetamodel.getInstance(), modus, metaData, daten, C_relationshipmetamodel.getInstance());
