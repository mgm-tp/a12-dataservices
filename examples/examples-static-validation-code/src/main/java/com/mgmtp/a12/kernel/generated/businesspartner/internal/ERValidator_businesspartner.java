...
public class ERValidator_businesspartner
	...
	private static final IMetaDataValidierung metaData = Meta_businesspartner.getInstance().getMetaDataValidierung();
	private static final ERValidator_businesspartner instance = new ERValidator_businesspartner();

	private ERValidator_businesspartner () {
	}

	public static ERValidator_businesspartner getInstance() {
		return instance;
	}
	...
	private void pruefeParams(
		...
		if (modus != null && !Meta_businesspartner.getInstance().isSpracheUnterstuetzt(modus.getLanguage())) {
	...
	public void validatePartlyWith3ValueLogic(
		...
		ValidationCommand c = new ValidationCommand(R_businesspartner.getInstance(), modus, metaData, daten);
	...
	public ICalculationResult calculateAll(IData daten, ValidationMode modus, IResult ergebnis) {
		pruefeParams(daten, ergebnis, modus);
		CalculationCommand c = new CalculationCommand(R_businesspartner.getInstance(), modus, metaData, daten, C_businesspartner.getInstance());
