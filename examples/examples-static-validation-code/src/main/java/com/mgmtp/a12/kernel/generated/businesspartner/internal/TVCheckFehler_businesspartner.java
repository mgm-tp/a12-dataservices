 ...
 *  
 */
public class TVCheckFehler_businesspartner implements ITeilValidierungChecker {

	private R_businesspartner checkObject = R_businesspartner.getInstance();

	public TVCheckFehler_businesspartner() {
	}

	public void fuehreAus(MainValidatorController controller, int interneFeldNr, long[] indices) {
		if(interneFeldNr<=29) { fuehreAus_0(controller, interneFeldNr, indices); }
	...
	private void fuehreAus_0(MainValidatorController controller, int interneFeldNr, long[] indices) {
		...
		default:
			if(interneFeldNr<=29) {
				...
				throw new IllegalArgumentException("Die interne Feldnummer "+interneFeldNr+" liegt nicht im Interval [1,29].");
			}
		}
	}
