...
public class ERValidator_contractcdm
	...
	private static final IMetaDataValidierung metaData = Meta_contractcdm.getInstance().getMetaDataValidierung();
	private static final ERValidator_contractcdm instance = new ERValidator_contractcdm();

	private ERValidator_contractcdm () {
	}

	public static ERValidator_contractcdm getInstance() {
		return instance;
	}
	...
	private void pruefeParams(
		...
		if (modus != null && !Meta_contractcdm.getInstance().isSpracheUnterstuetzt(modus.getLanguage())) {
	...
	public void validatePartlyWith3ValueLogic(
		...
		ValidationCommand c = new ValidationCommand(R_contractcdm.getInstance(), modus, metaData, daten);
	...
	public ICalculationResult calculateAll(IData daten, ValidationMode modus, IResult ergebnis) {
		pruefeParams(daten, ergebnis, modus);
		CalculationCommand c = new CalculationCommand(R_contractcdm.getInstance(), modus, metaData, daten, C_contractcdm.getInstance());
