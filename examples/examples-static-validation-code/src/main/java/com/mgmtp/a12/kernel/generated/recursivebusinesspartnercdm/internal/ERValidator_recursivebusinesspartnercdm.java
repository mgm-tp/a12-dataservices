...
public class ERValidator_recursivebusinesspartnercdm
	...
	private static final IMetaDataValidierung metaData = Meta_recursivebusinesspartnercdm.getInstance().getMetaDataValidierung();
	private static final ERValidator_recursivebusinesspartnercdm instance = new ERValidator_recursivebusinesspartnercdm();

	private ERValidator_recursivebusinesspartnercdm () {
	}

	public static ERValidator_recursivebusinesspartnercdm getInstance() {
		return instance;
	}
	...
	private void pruefeParams(
		...
		if (modus != null && !Meta_recursivebusinesspartnercdm.getInstance().isSpracheUnterstuetzt(modus.getLanguage())) {
	...
	public void validatePartlyWith3ValueLogic(
		...
		ValidationCommand c = new ValidationCommand(R_recursivebusinesspartnercdm.getInstance(), modus, metaData, daten);
	...
	public ICalculationResult calculateAll(IData daten, ValidationMode modus, IResult ergebnis) {
		pruefeParams(daten, ergebnis, modus);
		CalculationCommand c = new CalculationCommand(R_recursivebusinesspartnercdm.getInstance(), modus, metaData, daten, C_recursivebusinesspartnercdm.getInstance());
