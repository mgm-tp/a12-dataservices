...
public class ERValidator_address
	...
	private static final IMetaDataValidierung metaData = Meta_address.getInstance().getMetaDataValidierung();
	private static final ERValidator_address instance = new ERValidator_address();

	private ERValidator_address () {
	}

	public static ERValidator_address getInstance() {
		return instance;
	}
	...
	private void pruefeParams(
		...
		if (modus != null && !Meta_address.getInstance().isSpracheUnterstuetzt(modus.getLanguage())) {
	...
	public void validatePartlyWith3ValueLogic(
		...
		ValidationCommand c = new ValidationCommand(R_address.getInstance(), modus, metaData, daten);
	...
	public ICalculationResult calculateAll(IData daten, ValidationMode modus, IResult ergebnis) {
		pruefeParams(daten, ergebnis, modus);
		CalculationCommand c = new CalculationCommand(R_address.getInstance(), modus, metaData, daten, C_address.getInstance());
