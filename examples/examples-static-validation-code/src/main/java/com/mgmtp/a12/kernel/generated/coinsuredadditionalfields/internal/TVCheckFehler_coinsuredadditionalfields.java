 ...
 *  
 */
public class TVCheckFehler_coinsuredadditionalfields implements ITeilValidierungChecker {

	private R_coinsuredadditionalfields checkObject = R_coinsuredadditionalfields.getInstance();

	public TVCheckFehler_coinsuredadditionalfields() {
	}

	public void fuehreAus(MainValidatorController controller, int interneFeldNr, long[] indices) {
		if(interneFeldNr<=1) { fuehreAus_0(controller, interneFeldNr, indices); }
	...
	private void fuehreAus_0(MainValidatorController controller, int interneFeldNr, long[] indices) {
		...
		default:
			if(interneFeldNr<=1) {
				...
				throw new IllegalArgumentException("Die interne Feldnummer "+interneFeldNr+" liegt nicht im Interval [1,1].");
			}
		}
	}
