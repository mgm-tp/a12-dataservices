...
public class ERValidator_contract
	...
	private static final IMetaDataValidierung metaData = Meta_contract.getInstance().getMetaDataValidierung();
	private static final ERValidator_contract instance = new ERValidator_contract();

	private ERValidator_contract () {
	}

	public static ERValidator_contract getInstance() {
		return instance;
	}
	...
	private void pruefeParams(
		...
		if (modus != null && !Meta_contract.getInstance().isSpracheUnterstuetzt(modus.getLanguage())) {
	...
	public void validatePartlyWith3ValueLogic(
		...
		ValidationCommand c = new ValidationCommand(R_contract.getInstance(), modus, metaData, daten);
	...
	public ICalculationResult calculateAll(IData daten, ValidationMode modus, IResult ergebnis) {
		pruefeParams(daten, ergebnis, modus);
		CalculationCommand c = new CalculationCommand(R_contract.getInstance(), modus, metaData, daten, C_contract.getInstance());
