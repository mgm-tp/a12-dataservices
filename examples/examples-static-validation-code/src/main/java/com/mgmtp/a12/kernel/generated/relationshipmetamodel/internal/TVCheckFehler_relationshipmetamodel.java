 ...
 *  
 */
public class TVCheckFehler_relationshipmetamodel implements ITeilValidierungChecker {

	private R_relationshipmetamodel checkObject = R_relationshipmetamodel.getInstance();

	public TVCheckFehler_relationshipmetamodel() {
	}

	public void fuehreAus(MainValidatorController controller, int interneFeldNr, long[] indices) {
		if(interneFeldNr<=22) { fuehreAus_0(controller, interneFeldNr, indices); }
	...
	private void fuehreAus_0(MainValidatorController controller, int interneFeldNr, long[] indices) {
		...
		default:
			if(interneFeldNr<=22) {
				...
				throw new IllegalArgumentException("Die interne Feldnummer "+interneFeldNr+" liegt nicht im Interval [1,22].");
			}
		}
	}
