...
public class ERValidator_businesspartnerltd
	...
	private static final IMetaDataValidierung metaData = Meta_businesspartnerltd.getInstance().getMetaDataValidierung();
	private static final ERValidator_businesspartnerltd instance = new ERValidator_businesspartnerltd();

	private ERValidator_businesspartnerltd () {
	}

	public static ERValidator_businesspartnerltd getInstance() {
		return instance;
	}
	...
	private void pruefeParams(
		...
		if (modus != null && !Meta_businesspartnerltd.getInstance().isSpracheUnterstuetzt(modus.getLanguage())) {
	...
	public void validatePartlyWith3ValueLogic(
		...
		ValidationCommand c = new ValidationCommand(R_businesspartnerltd.getInstance(), modus, metaData, daten);
	...
	public ICalculationResult calculateAll(IData daten, ValidationMode modus, IResult ergebnis) {
		pruefeParams(daten, ergebnis, modus);
		CalculationCommand c = new CalculationCommand(R_businesspartnerltd.getInstance(), modus, metaData, daten, C_businesspartnerltd.getInstance());
