...
@Slf4j
public class AttachmentEndpointEnabledCondition extends AbstractDataServicesCondition {

	private static final String ATTACHMENT_ENDPOINT_ENABLED = "mgmtp.a12.dataservices.attachments.rest-endpoint.enabled";
	private static final String ATTACHMENT_ENABLED = "mgmtp.a12.dataservices.attachments.enabled";
	...
	@Override protected boolean evaluateCondition(DataServicesCoreProperties dataServicesCoreProperties) {
		return dataServicesCoreProperties.getAttachments().getRestEndpoint().isEnabled() &&
			dataServicesCoreProperties.getAttachments().isEnabled();
	}
	...
	private ConfigurationMessage enabledMessage() {
		return makeValidMessage("Attachment REST endpoint enabled", ATTACHMENT_ENDPOINT_ENABLED, ATTACHMENT_ENABLED);
	}

	private ConfigurationMessage disabledMessage() {
		return makeValidMessage("Attachment REST endpoint is disabled", ATTACHMENT_ENDPOINT_ENABLED, ATTACHMENT_ENABLED);
	}
}
