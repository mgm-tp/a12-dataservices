...
public class SmeWorkspaceImportEnabledCondition extends AbstractSmeWorkspaceCondition {
	private static final String IMPORT_ENABLED_PROPERTY_NAME = DataServicesCoreProperties.PROPERTIES_PREFIX + SME_WORKSPACE_PROPERTY_PATH + ".import.enabled";

	@Override protected boolean isEnabled(SmeWorkspaceProperties workspaceProperties) {
		return workspaceProperties.getImports().isEnabled();
	}

	@Override protected String getProperty() {
		return IMPORT_ENABLED_PROPERTY_NAME;
	}

	@Override protected String getDisabledMessage() {
		return "SME Workspace importing is disabled";
	}

	@Override protected String getEnabledMessage() {
		return "SME Workspace importing is enabled";
	}
}
