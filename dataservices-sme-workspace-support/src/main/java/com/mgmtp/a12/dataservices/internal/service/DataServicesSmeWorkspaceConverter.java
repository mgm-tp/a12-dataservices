...
@WcfConverter(order = 0) public class DataServicesSmeWorkspaceConverter implements WorkspaceConverter {
	@Override public Workspace convert(Workspace workspace) {
		workspace.getFiles().entrySet().stream()
			...
			.filter(DataServicesSmeWorkspaceConverter::belongsToDataServices)
			.forEach(f -> f.getValue().setOutputPath(f.getKey()));
		return workspace;
	}

	private static boolean belongsToDataServices(Map.Entry<String, FileTuple> f) {
