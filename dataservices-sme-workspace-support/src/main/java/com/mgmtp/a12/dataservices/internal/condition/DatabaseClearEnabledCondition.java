...
public class DatabaseClearEnabledCondition extends AbstractSmeWorkspaceCondition {
	private static final String DELETE_ENABLED_PROPERTY_NAME = DataServicesCoreProperties.PROPERTIES_PREFIX + SME_WORKSPACE_PROPERTY_PATH + ".delete.enabled";

	@Override protected boolean isEnabled(SmeWorkspaceProperties workspaceProperties) {
		return workspaceProperties.getDelete().isEnabled();
	}

	@Override protected String getProperty() {
		return DELETE_ENABLED_PROPERTY_NAME;
	}

	@Override protected String getDisabledMessage() {
		return "Database truncating disabled";
	}

	@Override protected String getEnabledMessage() {
		return "Database truncating enabled";
	}
}
