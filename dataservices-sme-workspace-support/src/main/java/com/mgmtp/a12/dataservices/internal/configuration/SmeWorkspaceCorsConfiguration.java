...
@Configuration
@AllArgsConstructor
public class SmeWorkspaceCorsConfiguration extends UAASecurityConfigurer<SmeWorkspaceCorsConfiguration> {
	...
	public void configureHttpSecurity(HttpSecurity http) {
		http.authorizeHttpRequests(authorizedHttpRequest ->
			authorizedHttpRequest.requestMatchers(PathPatternRequestMatcher.withDefaults()
				.matcher("%s/sme/workspace/**".formatted(authenticationProperties.getContextPath()))).permitAll());
